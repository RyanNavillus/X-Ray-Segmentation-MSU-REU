...
with open("./Training/{}.txt".format(uid)) as param_file:
    for line in param_file:
        if "Model" in line:
            hyperparams["model"] = line.split()[1]
        if "Backbone" in line:
            hyperparams["backbone"] = line.split()[1]
        if "Filter List" in line:
            filters = line.split()[2:]
            intfilters = [0]*len(filters)
            for i,f in enumerate(filters):
                intfilters[i] = int(re.sub("\D", "", f))
            hyperparams["unet_filters"] = tuple(intfilters)
        if "Filter count" in line:
            hyperparams["fpn_filters"] = line.split()[2]
        if "Spatial Dropout Rate" in line:
            hyperparams["fpn_dropout"] = line.split()[3]
        if "Batch Size" in line:
            hyperparams["batch_size"] = line.split()[2]
print(hyperparams)
...
model_backbone = hyperparams["backbone"]
if "unet_filters" in hyperparams:
    unet_filters = hyperparams["unet_filters"]
